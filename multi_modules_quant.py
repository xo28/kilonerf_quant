...
from quantize import Quantize, QParams, calculate_qparams
# from smart_exchange_1_peline import smart_net
# from yonggan.mobilenet import Scale_output, hook_layer, Scale_relu, hook_featuremap_in, hook_featuremap_out, hook_output
# from yonggan.se_weight_process import float2int, int2float, round_weight
import sys
# from multi_modules_infer import MultiNetworkLinear_infer
...
class MultiNetworkFourierEmbedding(nn.Module):
    ...
    def forward(self, x, implementation='pytorch', num_blocks=46, num_threads=512):
        ...
        if implementation == 'pytorch':
            x = x.unsqueeze(3).expand(self.num_networks, batch_size, num_input_channels,
                                      2 * self.num_frequencies + 1).contiguous()
            x[:, :, :, 1:1 + self.num_frequencies] = x[:, :, :, 0].unsqueeze(3) * self.frequency_bands.unsqueeze(
                0).unsqueeze(0).unsqueeze(0).to(x)
            ...
            x = kilonerf_cuda.compute_fourier_features(x.contiguous().view(-1), self.frequency_bands, num_blocks,
                                                       num_threads, implementation)
...
class SharedLinear(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=True, nonlinearity='leaky_relu',
                 weight_initialization_method='kaiming_uniform', bias_initialization_method='standard'):
...
def naive_multimatmul(biases, input_vectors, weights, out_features, in_features, batch_size_per_network):
    ...
    for network_index in range(num_networks):
        ...
        torch.addmm(biases[network_index], input_vectors[start_index:end_index], weights[network_index],
...
class AddMultiMatMul(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, biases, input_vectors, weights, out_features, in_features, batch_size_per_network, group_limits,
                aux_index, aux_index_backward):
        ...
        return kilonerf_cuda.multimatmul_magma_grouped_static(biases, input_vectors, weights,
                                                              out_features, in_features, batch_size_per_network, 4,
                                                              1024, group_limits, aux_index)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input_vectors = kilonerf_cuda.multimatmul_magma_grouped_static_without_bias_transposed_weights(biases,
                                                                                                            grad_output,
                                                                                                            weights,
                                                                                                            ctx.in_features,
                                                                                                            ctx.out_features,
...
class MultiNetworkLinear(nn.Module):
    ...
    def __init__(self, num_networks, in_features, out_features, nonlinearity='leaky_relu',
                 bias=True, implementation='bmm', nonlinearity_params=None,
                 use_same_initialization_for_all_networks=False,
                 network_rng_seed=None, weight_initialization_method='kaiming_uniform',
        ...
        if self.implementation.startswith('multimatmul'):
            ...
            self.aux_index = kilonerf_cuda.init_multimatmul_magma_grouped(self.num_networks, self.out_features,
            ...
            if self.implementation == 'multimatmul_differentiable':
                ...
                self.aux_index_backward = kilonerf_cuda.init_multimatmul_magma_grouped(self.num_networks,
                                                                                       self.in_features,
                                                                                       self.out_features,
                                                                                       self.group_limits)
        self.nonlinearity = nonlinearity
        ...
        self.register_buffer('M0_input', torch.zeros((1, 1, 1)))
        self.register_buffer('n_input', torch.zeros(()))
        self.register_buffer('M0_weight', torch.zeros((1, 1, 1)))
    ...
    def reset_parameters(self):
        ...
        if self.nonlinearity != 'sine':
            ...
            c, w0, is_first = self.nonlinearity_params['c'], self.nonlinearity_params['w0'], self.nonlinearity_params[
                'is_first']
        ...
        if 'multimatmul' in self.implementation:
            self.weight.data = self.weight.data.view(self.num_networks, self.in_features,
                                                     self.out_features).contiguous()

    def forward(self, x, batch_size_per_network=None, bias=None, weight=None, num_bits=8):
        ...
        if self.implementation == 'multimatmul':
            ...
            return kilonerf_cuda.multimatmul_magma_grouped_static(self.bias, x.contiguous(), self.weight,
                                                                  ...
                                                                  batch_size_per_network, 4, 1024, self.group_limits,
                                                                  self.aux_index)
        elif self.implementation == 'multimatmul_differentiable':
            if self.training:
                qparams = calculate_qparams(
                    x, num_bits=num_bits + 1, flatten_dims=(1, -1), reduce_dim=0, reduce_type='extreme')
                with torch.no_grad():
            ...
            qinput, M0_input, n_input, scale_input = Quantize(x, num_bits=num_bits, qparams=qparams, dequantize=True,
            ...
            return AddMultiMatMul.apply(qbias, qinput.contiguous(), qweight, self.out_features, self.in_features,
                                        batch_size_per_network, self.group_limits,
                                        self.aux_index, self.aux_index_backward)

        elif self.implementation == 'naive_multimatmul_differentiable':
            return naive_multimatmul_differentiable(self.bias, x, self.weight, self.out_features, self.in_features,
                                                    batch_size_per_network)
        else:
...
class MultiNetwork(nn.Module):
    def __init__(self, num_networks, num_position_channels, num_direction_channels, num_output_channels,
                 hidden_layer_size, num_hidden_layers, refeed_position_index=None, late_feed_direction=False,
                 direction_layer_size=None, nonlinearity='relu', nonlinearity_initalization='pass_leaky_relu',
                 use_single_net=False, linear_implementation='bmm', use_same_initialization_for_all_networks=False,
                 network_rng_seed=None, weight_initialization_method='kaiming_uniform',
                 bias_initialization_method='standard', alpha_rgb_initalization='updated_yenchenlin',
        ...
        self.register_buffer('directions_scale_input', torch.zeros(()))
        ...
        def linear_layer(in_features, out_features, actual_nonlinearity, use_hard_parameter_sharing=False):
            ...
            if not use_hard_parameter_sharing:
                return MultiNetworkLinear(self.num_networks, in_features, out_features,
                                          nonlinearity=passed_nonlinearity, nonlinearity_params=nonlinearity_params,
        ...
        if self.late_feed_direction:
            ...
            for i in range(self.num_hidden_layers - 1):
                if i == self.refeed_position_index:
                    new_layer = linear_layer(self.hidden_layer_size + self.num_position_channels,
            ...
            self.direction_layer = linear_layer(self.num_direction_channels + self.hidden_layer_size,
            ...
            if not self.use_view_independent_color:
                self.alpha_linear = linear_layer(self.hidden_layer_size, 1,
                                                 'linear' if self.alpha_rgb_initalization == 'updated_yenchenlin' else 'relu')
            self.rgb_linear = linear_layer(self.direction_layer_size, 3,
            ...
            self.view_dependent_parameters = list(self.direction_layer.parameters()) + list(
                self.rgb_linear.parameters())  # needed for L2 regularization only on the view-dependent part of the network
            ...
            layers = [linear_layer(self.num_position_channels + self.num_direction_channels, self.hidden_layer_size),
                      self.activation]
            nonlinearity_params = nonlinearity_params.copy().update({'is_first': False})
        ...
        # self.hook_fm_in1 = hook_featuremap_in()
        # self.hook_fm_in2 = hook_featuremap_in()
        # self.hook_fm_in3 = hook_featuremap_in()
        # self.hook_fm_in4 = hook_featuremap_in()
        # self.hook_fm_in5 = hook_featuremap_in()
        # self.hook_fm_in6 = hook_featuremap_in()
        # self.hook_fm_in7 = hook_featuremap_in()
        # self.hook_fm_in8 = hook_featuremap_in()
        # self.hook_fm_in9 = hook_featuremap_in()
    ...
    def serialize_params(self):
        ...
        self.serialized_params.append(
            process_weight(torch.cat([self.alpha_linear.weight, self.feature_linear.weight], dim=2)))
    ...
    def forward(self, x, batch_size_per_network=None, random_directions=None):
        if self.late_feed_direction:
            if isinstance(x, list):
                ...
                positions = self.hook_fm_in1(positions)
                directions = self.hook_fm_in2(directions)
                # frees memory of inputs
                x[0] = None
                x[1] = None
            else:
                positions, directions = torch.split(x, [self.num_position_channels, self.num_direction_channels],
                                                    dim=-1)
            h = positions
            directions, M0_input, n_input, directions_scale_input = Quantize(directions, num_bits=8, qparams=None, dequantize=True,
                                                              stochastic=False, inplace=False)
            self.directions_scale_input.data = directions_scale_input.data

            # for i, l in enumerate(self.pts_linears):
            h = self.pts_linears[0](h, batch_size_per_network)
            PerfMonitor.add('pts_linears ' + str(0), ['network query', 'matmul'])
            h = self.activation(h)
            PerfMonitor.add('activation ' + str(0), ['network query', 'matmul'])
            if 0 == self.refeed_position_index:
                h = torch.cat([positions, h], -1)
                PerfMonitor.add('cat[positions, h]', ['network query', ])
            h = self.pts_linears[1](h, batch_size_per_network)
            PerfMonitor.add('pts_linears ' + str(1), ['network query', 'matmul'])
            h = self.activation(h)
            PerfMonitor.add('activation ' + str(1), ['network query', 'matmul'])
            if 1 == self.refeed_position_index:
                h = torch.cat([positions, h], -1)
                PerfMonitor.add('cat[positions, h]', ['network query', ])
    ...
    def extract_single_network(self, network_index):
        single_network = MultiNetwork(1, self.num_position_channels, self.num_direction_channels,
                                      self.num_output_channels,
                                      self.hidden_layer_size, self.num_hidden_layers, self.refeed_position_index,
                                      self.late_feed_direction,
                                      self.direction_layer_size, self.nonlinearity, self.nonlinearity_initalization,
                                      self.use_single_net,
...
def query_multi_network(multi_network, domain_mins, domain_maxs, points, directions,
    ...
    network_strides = torch.tensor([res[2] * res[1], res[2], 1], dtype=torch.long,
    ...
    if 'block_size' in cfg:
        ...
        for i in range(assigned_networks_per_block.shape[0]):
            for j in range(0, assigned_networks_per_block.shape[-1], cfg['block_size'][2]):
                unique_assigned_networks, counts = torch.unique(
                    assigned_networks_per_block[i, :, :, j:j + cfg['block_size'][2]], return_counts=True)
    ...
    if use_network_jittering:
        ...
        local_points = 2 * (local_points - domain_mins_reordered) / (
                    domain_maxs_reordered - domain_mins_reordered) - 1  # coords in [-1, 1]
        ...
        del local_points
        pos_dist = cfg['network_jitter']['prob_at_border'] * torch.exp(
            -cfg['network_jitter']['dropoff_rate'] * pos_dist)
        neg_dist = cfg['network_jitter']['prob_at_border'] * torch.exp(
            -cfg['network_jitter']['dropoff_rate'] * neg_dist)
        ...
        filtered_point_indices = (filtered_point_indices_3d * network_strides).sum(
            dim=1)  # convert to flat indices again
...
def build_multi_network_from_single_networks(single_networks, transpose_weight=True,
                                             linear_implementation='multimatmul',
    ...
    multi_network = MultiNetwork(num_networks, p.num_position_channels, p.num_direction_channels, p.num_output_channels,
                                 p.hidden_layer_size, p.num_hidden_layers, p.refeed_position_index,
                                 p.late_feed_direction,
                                 p.direction_layer_size, p.nonlinearity, linear_implementation=linear_implementation,
                                 use_hard_parameter_sharing_for_color=use_hard_parameter_sharing_for_color,
                                 view_dependent_dropout_probability=view_dependent_dropout_probability,
...
def test():
    ...
    multi_network = MultiNetwork(num_networks, num_input_channels, num_output_channels, hidden_layer_size,
                                 num_hidden_layers).to(dev)
        ...
        for network_index in range(num_networks):
            ...
            for multi_layer, single_layer in zip(multi_network.layers, single_network):
                if isinstance(single_layer, nn.Linear):
                    parameters_matching = parameters_matching and torch.allclose(single_layer.weight.data,
                                                                                 multi_layer.weight.data[network_index]) \
                                          and torch.allclose(single_layer.bias.data,
                                                             multi_layer.bias.data[network_index])
    ...
    start_time = perf_counter()
    loss_func = nn.MSELoss(
        reduction='none')  # we need to reduce it ourselves: average over output channel (2) and batch dimension (1) but sum over network dimension (0)
    ...
    print('single network time: {}, multi network time: {}, speedup: {}, theoretical max speedup: {}'.format(
        single_network_duration, multi_network_duration, single_network_duration / multi_network_duration,
        num_networks))
